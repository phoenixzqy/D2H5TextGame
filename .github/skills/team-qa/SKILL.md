...
Present the qa-lead's full strategy to the user, then use `ask_user`:
...
Use `ask_user` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `ask_user` for each:
...
After each FAIL result: use `ask_user` to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
