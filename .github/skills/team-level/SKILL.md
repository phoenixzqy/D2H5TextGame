...
> Translated mechanically for Copilot CLI: `AskUserQuestion` → `ask_user` tool;
> `Task` subagent invocations → our `agent` tool. Some agent names referenced below
> (`lead-programmer`, `qa-lead`, `gameplay-programmer`, etc.) live in
> `docs/templates/agents/optional/` — activate via `setup-engine` skill if needed.
> The active D2 H5 text-game fleet is documented in `AGENTS.md`; map any reference
> to the closest active equivalent (e.g. `lead-programmer` → `technical-director` +
> `reviewer`; `qa-lead` → `qa-engineer`; `gameplay-programmer` → `engine-dev`).

When this skill is invoked:

**Decision Points:** At each step transition, use `ask_user` to present
...
**Gate**: Use `ask_user` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
**Gate**: Use `ask_user` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `ask_user` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `ask_user` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
3. **Offer options** via ask_user with choices:
