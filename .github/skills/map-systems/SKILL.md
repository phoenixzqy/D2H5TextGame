...
Then use `ask_user` to capture feedback:
...
Use `ask_user` to ask: "Does this dependency ordering look right? Any
...
Use `ask_user` to ask: "Do these priority assignments match your vision?
...
Use `ask_user` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `ask_user`:
...
After the systems index is created (or after designing some systems), present next actions using `ask_user`:
...
2. **ask_user** at every decision point (Explain -> Capture pattern):
