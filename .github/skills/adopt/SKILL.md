...
If the project appears fresh (no artifacts at all), use `ask_user`:
...
**If it does not exist**: Use `ask_user`:
...
and offer to handle it immediately using `ask_user`. Choose the first
branch that applies:
