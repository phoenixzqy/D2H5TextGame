...
import type { RegisteredSkill, SkillEffect, SummonMaxCountScalingKind } from './effects';
...
interface BuildSpec {
  ...
  readonly summon?: {
    ...
    readonly maxCountScaling?: SummonMaxCountScalingKind;
  };
...
function build(s: BuildSpec): RegisteredSkill {
  ...
  return {
    ...
    ...(s.aoeShape ? { aoeShape: s.aoeShape } : {}),
    ...(s.buff ? { isBuff: true } : {}),
...
export const DEFAULT_SKILLS: readonly RegisteredSkill[] = Object.freeze([
  ...
  build({ id: 'sorceress.frozen_orb', archetype: 'sorceress', target: 'area-enemies', cooldown: 2, manaCost: 30, damageType: 'cold', base: [60, 90], status: { id: 'chill' }, aoeShape: 'box-3x3' }),
  build({ id: 'sorceress.chain_lightning', archetype: 'sorceress', target: 'area-enemies', cooldown: 2, manaCost: 25, damageType: 'lightning', base: [50, 70], status: { id: 'paralyze', chance: 0.2 }, aoeShape: 'line-3' }),
  ...
  build({ id: 'sorceress.meteor', archetype: 'sorceress', target: 'area-enemies', cooldown: 4, manaCost: 50, damageType: 'fire', base: [100, 150], status: { id: 'ignite', dotPct: 0.1 }, aoeShape: 'box-3x3' }),
  build({ id: 'sorceress.blizzard', archetype: 'sorceress', target: 'area-enemies', cooldown: 3, manaCost: 45, damageType: 'cold', base: [80, 120], status: { id: 'chill' }, aoeShape: 'box-3x3', minLevel: 24 }),
  ...
  build({
    ...
    cooldown: 0,
    manaCost: 15,
    summon: { summonId: 'skeleton', max: 3, onStart: true, maxCountScaling: 'raise-skeleton-1-6-12-cap-3' }
  }),
  build({ id: 'necromancer.clay_golem', archetype: 'necromancer', target: 'summon', cooldown: 3, manaCost: 15, summon: { summonId: 'clay_golem', max: 1, onStart: true }, minLevel: 6 }),
  build({ id: 'necromancer.blood_golem', archetype: 'necromancer', target: 'summon', cooldown: 4, manaCost: 25, summon: { summonId: 'blood_golem', max: 1, onStart: true }, minLevel: 18 }),
  build({ id: 'necromancer.iron_golem', archetype: 'necromancer', target: 'summon', cooldown: 4, manaCost: 35, summon: { summonId: 'iron_golem', max: 1, onStart: true }, minLevel: 24 }),
  build({ id: 'necromancer.fire_golem', archetype: 'necromancer', target: 'summon', cooldown: 4, manaCost: 35, summon: { summonId: 'fire_golem', max: 1, onStart: true }, minLevel: 30 }),
  ...
  // Paladin
  build({ id: 'paladin.zeal', archetype: 'paladin', target: 'area-enemies', cooldown: 1, manaCost: 10, damageType: 'physical', base: [40, 60], aoeShape: 'cross' }),
  ...
  build({ id: 'amazon.multiple_shot', archetype: 'amazon', target: 'area-enemies', cooldown: 2, manaCost: 15, damageType: 'physical', base: [40, 60], aoeShape: 'cross' }),
  build({ id: 'amazon.freezing_arrow', archetype: 'amazon', target: 'area-enemies', cooldown: 2, manaCost: 20, damageType: 'cold', base: [50, 70], status: { id: 'chill' }, aoeShape: 'box-3x3' }),
  ...
  build({ id: 'amazon.plague_javelin', archetype: 'amazon', target: 'area-enemies', cooldown: 2, manaCost: 25, damageType: 'poison', base: [30, 50], status: { id: 'plague', stacks: 3, dotPct: 0.05 }, aoeShape: 'line-3' }),
  ...
  build({ id: 'druid.arctic_blast', archetype: 'druid', target: 'area-enemies', cooldown: 2, manaCost: 25, damageType: 'cold', base: [50, 70], status: { id: 'chill' }, aoeShape: 'cross' }),
  build({ id: 'druid.tornado', archetype: 'druid', target: 'area-enemies', cooldown: 2, manaCost: 30, damageType: 'physical', base: [60, 90], aoeShape: 'line-3' }),
  ...
  build({ id: 'assassin.fire_blast', archetype: 'assassin', target: 'area-enemies', cooldown: 2, manaCost: 25, damageType: 'fire', base: [50, 70], status: { id: 'ignite', dotPct: 0.1 }, aoeShape: 'cross' }),
  build({ id: 'assassin.blade_fury', archetype: 'assassin', target: 'area-enemies', cooldown: 1, manaCost: 15, damageType: 'physical', base: [40, 60], aoeShape: 'line-3' }),
